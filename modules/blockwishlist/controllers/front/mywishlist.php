...
class BlockWishListMyWishListModuleFrontController extends ModuleFrontController
{
            ...
            if (Tools::isSubmit('submitWishlist')) {
                ...
                if (!count($errors)) {
                    ...
                    if (!count($errors)) {
                        ...
                        srand($s * $us);
                        $wishlist->token = Tools::strtoupper(Tools::substr(sha1(uniqid(rand(), true)._COOKIE_KEY_.
                            $this->context->customer->id), 0, 16));
                        $wishlist->add();
                        Mail::Send(
                ...
                WishList::addCardToWishlist($this->context->customer->id, Tools::getValue('
                    id_wishlist'), $this->context->language->id);
    ...
    public function ajaxProcessProductChangeWishlist()
    {
        ...
        if (!$id_product || !is_int($id_product_attribute) || !$quantity ||
            ...
            die(Tools::jsonEncode(array('success' => false, 'error' => $this->module->l('
                Error while moving product to another list', 'mywishlist'))));
        }
        ...
        die(Tools::jsonEncode(array('success' => true, 'msg' => $this->module->l('
            The product has been correctly moved', 'mywishlist'))));
    }
}
