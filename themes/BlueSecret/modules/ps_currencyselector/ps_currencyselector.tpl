...
<div id="_desktop_currency_selector" class="hidden-md-down">
  <div class="currency-selector dropdown js-dropdown">
    <span id="currency-selector-label" class="hidden-sm-down">{l s='Currency' d='Shop.Theme.Global'}</span>
    {* <button data-target="#" data-toggle="dropdown" class="hidden-sm-down btn-unstyle" aria-haspopup="true" aria-expanded="false" aria-label="{l s='Currency dropdown' d='Shop.Theme.Global'}">
      <svg width="18px" height="17px"><use xlink:href="#hcur"></use></svg><span class="expand-more _gray-darker">{$current_currency.iso_code} {$current_currency.sign}</span>
      <i class="fa fa-angle-down"></i>
    </button> *}
    <ul class="hidden-sm-down" aria-labelledby="currency-selector-label">
      {foreach from=$currencies item=currency}
      <li {if $currency.current} class="current" {/if}><a title="{$currency.name}" rel="nofollow" href="{$currency.url}">{$currency.iso_code} {$currency.sign}</a></li>
      {/foreach}
    </ul>
